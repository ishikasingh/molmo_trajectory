...
The original version of this program can be found at https://github.com/stas00/ml-engineering/blob/master/network/benchmarks/all_reduce_bench.py
...
def timed_allreduce(mat: torch.Tensor, start_event: torch.cuda.Event, end_event: torch.cuda.Event) -> torch.Tensor:
    dist.barrier()
    start_event.record()
    dist.all_reduce(mat)
    end_event.record()
    ...
    size = mat.numel() * 4  # fp32 = 4 bytes per element

    # note that this is following the same math as NVIDIA/nccl-tests
    algbw = torch.tensor([size / duration], device="cuda")
...
@click.command()
@click.option("--backend", type=str, default="nccl")
@click.option("--numel", type=float, default=1e9)
@click.option("--trials", type=int, default=5)
def main(backend: str, numel: float, trials: int) -> None:
    local_rank = int(os.environ.get("LOCAL_RANK", "0"))
    torch.cuda.set_device(local_rank)
    dist.init_process_group(backend=backend)
    print(f"Initialized rank {dist.get_rank()} / {dist.get_world_size()}")

    is_global_rank_0 = dist.get_rank() == 0

    mat = torch.rand(int(numel), dtype=torch.float32, device="cuda")
    ...
    # real benchmark
    algbw_gather: list[torch.Tensor] = []
    for _ in tqdm.trange(trials, disable=local_rank > 0):
        algbw_gather.append(timed_allreduce(mat, start_event, end_event))

    algbw = torch.mean(torch.cat(algbw_gather))
    ...
    if is_global_rank_0:
        print(
            f"The average bandwidth of all_reduce with a {numel * 4/1e9}GB payload ({trials} trials, {n} ranks):\n",
